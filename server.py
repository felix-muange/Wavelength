...
# Allow cross-origin requests to prevent ERR_FAILED
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def _extract_audio_sync(video_id: str):
    with YoutubeDL(YDL_OPTS) as ydl:
        try:
            ...
            # Prioritize m4a for Chrome/Desktop compatibility
...
@app.get("/health")
async def health():
    return {"status": "ok", "client": "Chrome/Desktop"}
...
async def search_api(q: str = Query(...)):
    ...
    return await loop.run_in_executor(executor, _search, q)
...
async def stream(request: Request, videoId: str = Query(...)):
    ...
    async def byte_generator():
        async with aiohttp.ClientSession() as session:
            ext_headers = {'User-Agent': 'Mozilla/5.0'}
            if range_header: ext_headers['Range'] = range_header
            async with session.get(info['url'], headers=ext_headers) as resp:
